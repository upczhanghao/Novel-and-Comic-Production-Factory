    ...
    <div v-if="props.open" class="onb-mask" @click.self="skip">
      <div class="onb-modal">
        ...
        <main class="onb-body">
          <section v-if="step === 1">
            <h3>步骤 1：模型配置</h3>
            <p class="onb-tip">至少配置一个 LLM 才能开始创作。Embedding 与图片配置可后续添加。</p>
            <ul class="onb-status">
              <li :class="{ ok: hasLLM }">
                <span>LLM 配置</span>
              ...
              <li :class="{ ok: configStore.embeddingChoices.length > 0 }">
                ...
                <strong>{{ configStore.embeddingChoices.length > 0 ? '已配置' : '可选' }}</strong>
              </li>
              <li :class="{ ok: configStore.imageChoices.length > 0 }">
                <span>图片配置</span>
                <strong>{{ configStore.imageChoices.length > 0 ? '已配置' : '可选' }}</strong>
              </li>
            </ul>
        ...
        <footer class="onb-footer">
          ...
          <button v-else class="btn-primary" :disabled="!canFinish" @click="finish" type="button">进入工作台</button>
        </footer>
      </div>
    </div>
  </Teleport>
