...
<template>
  <Teleport to="body">
    <div class="fb-stack">
      <button v-if="feedback.items.length > 1" class="fb-clear" @click="feedback.dismissAll()" type="button">
        清除全部 ({{ feedback.items.length }})
      </button>
      <TransitionGroup name="fb">
        ...
        >
          ...
          <div class="fb-body">
            <div class="fb-msg">
              {{ item.message }}
              <span v-if="(item.count ?? 1) > 1" class="fb-count">×{{ item.count }}</span>
            </div>
...
<style scoped>
.fb-stack { position: fixed; bottom: 80px; right: 20px; z-index: 60; display: flex; flex-direction: column; gap: 8px; max-width: 380px; pointer-events: none; align-items: flex-end; }
.fb-clear { pointer-events: auto; align-self: flex-end; padding: 4px 10px; font-size: 11px; background: rgba(0,0,0,0.7); color: white; border-radius: 999px; border: 0; cursor: pointer; }
.fb-clear:hover { background: rgba(0,0,0,0.85); }
.fb-count { display: inline-block; margin-left: 6px; padding: 0 6px; font-size: 11px; background: var(--color-surface-muted); color: var(--color-ink-light); border-radius: 999px; }
